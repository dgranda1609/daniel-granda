...
const TOOLS = [
  'After Effects', 'Premiere Pro', 'DaVinci Resolve', 'Figma', 'n8n',
  'OpenAI API', 'Midjourney', 'Python', 'React', 'Runway Gen-3',
  'ComfyUI', 'Frame.io', 'Audition', 'Lightroom',
];
...
const AnimatedStat: React.FC<{ value: number; suffix: string; label: string; inView: boolean }> = ({ value, suffix, label, inView }) => {
...
export const About: React.FC = () => {
  ...
  return (
    <section id="agency" className="pb-12 md:pb-24 pt-32 md:pt-48 px-24 md:px-64 bg-brand-bg relative overflow-hidden">
      <div className="max-w-6xl mx-auto text-center reveal">
          ...
          <div className="relative">
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[200vw] overflow-hidden">
              <div className="flex animate-ticker items-center opacity-20 whitespace-nowrap">
                {TOOLS.map((tool, i) => (
                  <span key={`a-${i}`} className="text-24 md:text-32 font-bold text-brand-primary mx-24">{tool}</span>
                ))}
                {TOOLS.map((tool, i) => (
                  <span key={`b-${i}`} className="text-24 md:text-32 font-bold text-brand-primary mx-24">{tool}</span>
                ))}
              </div>
            </div>
        ...
        <div className="font-serif text-20 md:text-28 lg:text-32 leading-[1.4] opacity-90 mb-48 max-w-4xl mx-auto px-24 text-left space-y-24">
          <p>
            I'm Daniel Granda — a video producer and AI visual strategist based in South Florida. Currently leading media production at <strong className="text-brand-accent">Kreyol Essence</strong> (Shark Tank winner), shipping 100+ brand assets monthly through an AI-first pipeline that cut turnaround by 35%.
          </p>
          <p>
            Before that: a documentary series for the <strong>United Nations</strong> across three ecosystems. A <strong>Cannes World Film Festival</strong> finalist. 150+ videos at Miguel Angel Productions. Campaigns for <strong>Microsoft</strong>, <strong>The North Face</strong>, and America Television.
          </p>
          <p className="italic opacity-80">
            I use AI every day. But AI doesn't replace knowing when to cut, how to light, or why a story needs silence.
          </p>
        </div>

        {/* Animated Stats Row */}
        <div ref={statsRef} className="grid grid-cols-2 md:grid-cols-4 gap-32 md:gap-48 max-w-3xl mx-auto mb-48">
          {STATS.map((stat) => (
